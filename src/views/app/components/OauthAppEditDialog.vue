...
const oauthAppEdit = defineModel('oauthAppEdit', { required: true })
...
const oauthApp = ref(clone(oauthAppEdit.value))
...
const getOauthApp = async () => {
  oauthApp.value = clone(oauthAppEdit.value)
  if (oauthApp.value.scope) {
    scopeProfile.value = oauthApp.value.scope_list.includes('profile')
    scopeApi.value = oauthApp.value.scope_list.includes('api')
  }
}
